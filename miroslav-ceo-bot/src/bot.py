...
class MiroslavBot:
    ...
    async def _send_probe(self, context: ContextTypes.DEFAULT_TYPE, target_username: str | None = None):
        ...
        prompt = (
            f"Обратись к @{username} ({name}) в групповом чате. "
            f"Ты знаешь о нём: {facts_text}. "
            f"Задай ему 1-2 вопроса — но НЕ как на собеседовании. "
            f"Спроси что-то жизненное и конкретное, на что хочется ответить: "
            f"откуда он, чем занимается по жизни, какие у него хобби, "
            f"что он делал на выходных, есть ли у него мечта, и т.д. "
            f"Можешь подколоть, но вопрос должен быть таким, чтобы человек захотел ответить честно. "
            f"Не спрашивай про скиллы и работу напрямую — спроси про жизнь."
        )
