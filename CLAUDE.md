...
- Каждый час scheduler сжимает разговоры в bullet points через Claude
- Память инжектится в system prompt при каждом вызове Claude
- Лимит 10K символов, старые записи обрезаются автоматически
...
- Batch update каждый час — Claude анализирует сообщения и обновляет факты
